...
```json
{
    ...
    // status_id se asigna automáticamente como 1 (Borrador)
    "products": [
        {
            "product_type": "panel",
            "product_id": 2,
...
- `status_id` (integer, opcional, debe existir en la tabla quotation_statuses) - Se asigna automáticamente como 1 (Borrador) si no se envía
...
**Estado por Defecto:** Al crear una cotización, el estado se asigna automáticamente como 1 (Borrador) si no se especifica `status_id`.
